...
[[noreturn]] LONG CALLBACK Exception::ExceptionHandler(PEXCEPTION_POINTERS const pExs)
{
	...
	case 0xE06D7363: // exception thrown and not caught
	{
		...
		if (FILE* except = _wfsopen(except_file.c_str(), L"w", _SH_DENYNO)) {
			...
			fclose(except);
			LogInfo("Exception data has been saved to file:\n%ls\n", except_file.c_str());
		}
	...
	case ERROR_PROC_NOT_FOUND:
		LogInfo("Massive failure: Procedure or module not found!\n");
		break;
	default:
		LogInfo("Massive failure: reason unknown, have fun figuring it out\n");
		//			return EXCEPTION_CONTINUE_SEARCH;
		break;
	}
	...
	{
		LogInfo("Trying to save current map.\n");
		FA2::CString fcrash_backup = GlobalVars::ExePath();
	...
	GlobalVars::INIFiles::Rules->~INIClass();
	GlobalVars::INIFiles::AI->~INIClass();
	GlobalVars::INIFiles::Art->~INIClass();
	GlobalVars::INIFiles::Temperate->~INIClass();
	GlobalVars::INIFiles::Snow->~INIClass();
	GlobalVars::INIFiles::Urban->~INIClass();
	GlobalVars::INIFiles::CurrentDocument->~INIClass();
	GlobalVars::INIFiles::Sound->~INIClass();
	GlobalVars::INIFiles::Turtorial->~INIClass();
	GlobalVars::INIFiles::FAData->~INIClass();
	GlobalVars::INIFiles::FALanguage->~INIClass();
...
DEFINE_HOOK(435270, CFinalSunDlg_DoModal, 8)
{
	...
	LogInfo("FA2sp UnhandledExceptionFliter installed!\n");

	R->EAX(pThis->FA2::CDialog::DoModal());
