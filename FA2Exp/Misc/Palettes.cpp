...
std::map<FA2::CString, Palette*> Palettes::LoadedPalettes;
...
Palette* Palettes::LoadPalette(const FA2::CString& palName)
{
    ...
    if (BytePalette* pBuffer = (BytePalette*)GlobalVars::Dialog::CLoading->ReadWholeFile(palName))
    {
        auto pPal = FACreate<Palette>();
        for (int i = 0; i < 256; ++i)
        {
...
Palette* Palettes::GetRemap(const FA2::CString& palName, BGRStruct color)
{
