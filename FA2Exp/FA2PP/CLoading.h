...
public:
	...
	bool HasFile(FA2::CString filename);
	void SetTheaterLetter(FA2::CString& basename)
	{
		if (this->TheaterIdentifier != 0 && basename.GetLength() >= 2) {
			char c0 = basename[0];
			char c1 = basename[1] & ~0x20; // evil hack to uppercase, copied from Ares
			if (isalpha(static_cast<unsigned char>(c0))) {
				if (c1 == 'A' || c1 == 'T') {
					basename.SetAt(1, this->TheaterIdentifier);
				}
			}
		}
	}
	void SetGenericTheaterLetter(FA2::CString& string)
	{
		string.SetAt(1, 'G');
	}
