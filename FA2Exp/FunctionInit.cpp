...
{
	FA2::CString filepath = GlobalVars::FilePath();
	filepath += filename;
	...
	if (hFile != INVALID_HANDLE_VALUE)
	{
		...
		auto pBuffer = FACreateArray<unsigned char>(dwSize);
		if(pDwSize)
			*pDwSize = dwSize;
	...
	auto nMix = GlobalVars::Dialog::CLoading->SearchFile(filename);
	if (CMixFile::HasFile(filename, nMix))
	{
		...
		{
			auto pBuffer = FACreateArray<unsigned char>(pFile->FileSize);
...
bool CLoading::HasFile(FA2::CString filename)
{
	FA2::CString filepath = GlobalVars::FilePath();
	filepath += filename;
	HANDLE hFile = CreateFile(filepath, GENERIC_READ, FILE_SHARE_READ, nullptr,
